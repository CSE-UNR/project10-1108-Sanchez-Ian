...
//I attempted to pick a word from a file from multiple words.
//I tried to only use what I learned in class but online there were things that were in std library like rewind() that would have helped but since nothing of the sort was explained in class i did not use it.

#include <stdio.h>
#define WORD_LENGTH 5
#define MAX_GUESSES 6
#define READFILE "words.txt"
...
int filecheck(char word[]) {
	FILE *file = fopen(READFILE, "r");
		...
		char chosen[WORD_LENGTH + 1];
		int count = 0;
		
		while(fscanf(file, "%5s", temp) == 1){
			count++;
